...
export default function EditMemoryForm({ id }) {
  ...
  const handleUpdateMemory = async (event) => {
    ...
    try {
      setUpdating(true);
      await axios.put(`/api/memories/update-memory/${id}`, {
        title,
        description,
        image,
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3 text-base-content/60">
        ...
        <p className="text-sm font-medium">Loading memory details...</p>
      </div>
    );
  }
      ...
      {image && (
        <div className="p-4 bg-base-200/60 rounded-2xl border border-base-300">
          <p className="text-xs font-semibold text-base-content/60 mb-2 flex items-center gap-1.5">
            <ImageIcon className="w-4 h-4 text-primary" /> Preview:
          </p>
          <div className="relative w-full max-w-2xl mx-auto overflow-hidden rounded-2xl bg-base-300 border border-base-300 shadow-sm">
            <img
              src={image}
              alt="Memory preview"
